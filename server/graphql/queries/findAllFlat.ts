import { Id, IServiceCommon, IServiceGraphQL, StrapiGraphQLContext, ToBeFixed } from "../../../types";
...
export = ({ strapi, nexus }: StrapiGraphQLContext) => {
	...
	return {
		...
		args: {
			...
			filters: getPluginService<IServiceGraphQL>('gql').buildContentTypeFilters(contentType),
		...
		async resolve(obj: Object, args: ResponseFindAllResolverProps) {
			...
			return await getPluginService<IServiceCommon>('common')
				.findAllFlat(flatInput(
					relation, 
					getPluginService<IServiceGraphQL>('gql').graphQLFiltersToStrapiQuery(filters, contentType),
					sort,
					pagination ? {
						...pagination,
