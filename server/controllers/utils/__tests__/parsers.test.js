...
describe('Test Comments controller parsers utils', () => {
  ...
  describe('Flat Input properties', () => {
    ...
    test('Should contain default property \'populate\'', () => {
      expect(flatInput()).toHaveProperty(['populate', 'threadOf', 'populate', 'authorUser'], true);
    });

    test('Should assign relation to \'query\'', () => {
      expect(flatInput(relation)).toHaveProperty(['query', 'related'], relation);
    });

    test('Should assign filters to \'query\'', () => {
      const result = flatInput(relation, filters);
      expect(result).toHaveProperty(['query', 'content', '$eq'], filters.content['$eq']);
    });
    ...
    test('Should assign not allow to overwrite \'removed\' in $or operator', () => {
      ...
      expect(result).toHaveProperty(['query', '$or', 0, 'removed', '$null'], true);
    ...
    test('Should assign pagination', () => {
      ...
      expect(result).toHaveProperty(['pagination', 'pageSize'], pagination.pageSize);
    });
    ...
    test('Should build complex output', () => {
      ...
      const result = flatInput(relation, complexFilters, complexSort, pagination);

      expect(result).toHaveProperty(['query', 'related'], relation);
      expect(result).toHaveProperty(['query', '$or', 0, 'createdAt', '$lte'], oneDayBefore);
      expect(result).toHaveProperty(['query', '$or', 1, 'createdAt', '$eq'], now);
      expect(result).toHaveProperty(['query', '$or', 2, 'removed', '$null'], true);
      expect(result).toHaveProperty(['query', '$or', 3, 'removed'], false);
      expect(result).toHaveProperty(['query', 'content', '$contains'], complexFilters.content['$contains']);
      expect(result).toHaveProperty(['sort', 0, 'createdAt'], complexSort[0].createdAt);
