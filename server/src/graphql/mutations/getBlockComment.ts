...
export const getBlockComment = (strapi: CoreStrapi, nexus: Nexus) => {
  const { nonNull } = nexus;
  ...
  return {
    ...
    async resolve(_: object, args: { input: { id: string; relation: string } }) {
      ...
      return getPluginService(strapi, 'common').changeBlockedComment(unwrapEither(validated).commentId, true);
    },
  };
};
