...
const Settings = () => {
	...
	const { data: configData, isLoading: isConfigLoading, err: configErr }: ToBeFixed = fetch;

	const { data: allCollectionsData, isLoading: areCollectionsLoading, err: collectionsErr }: ToBeFixed = useQuery(
		['get-all-content-types', canAccess],
		() => fetchAllContentTypes(toggleNotification)
	  );

	const { data: allRolesData, isLoading: areRolesLoading, err: rolesErr }: ToBeFixed = useQuery(
	...
	const preparePayload = ({ enabledCollections, gqlAuthEnabled, approvalFlow, entryLabel, ...rest }: ToBeFixed) => ({
		...rest,
		enabledCollections,
	...
	const allCollections = !isLoading && allCollectionsData
		.filter(({ uid }: ToBeFixed) => first(uid.split(regexUID).filter(s => s && s.length > 0)) === 'api');
	const enabledCollections = configData?.enabledCollections
		?.map((uid: ToBeFixed) => allCollections.find(_ => _.uid === uid) ? uid : undefined)
	...
	const moderatorRoles = configData?.moderatorRoles
		?.filter((code: ToBeFixed) => allRoles.find((_: ToBeFixed) => _.code === code)) || [];

	const changeApprovalFlowFor = (uid: ToBeFixed, current: ToBeFixed, value: ToBeFixed) => {
	...
	const changeEntryLabelFor = (uid: ToBeFixed, current: ToBeFixed, value: ToBeFixed) => ({
		...current,
	...
	const handleSetContentTypeExpanded = (key: ToBeFixed) => setContentTypeExpanded(key === contentTypeExpanded ? undefined : key);
										...
										<Grid gap={4}>
											<GridItem col={12}>
												<Select
													...
													onChange={(value: ToBeFixed) => setFieldValue('enabledCollections', value, false)}
													disabled={restartRequired}
													multi
													withTags
												>
													{ allCollections.map(({ uid, schema: { displayName } }: ToBeFixed) => 
																...
																<Box padding={6}>
																	<Stack size={4}>
																		...
																		{ !isEmpty(stringAttributes) && (<Select
																			...
																			onChange={(value: ToBeFixed) => setFieldValue('entryLabel', changeEntryLabelFor(uid, values.entryLabel, value))}
																			multi
																			withTags
																			disabled={restartRequired}
																		>
										...
										<Grid gap={4}>
											<GridItem col={6} xs={12}>
												<Select
													...
													onChange={(value: ToBeFixed) => setFieldValue('moderatorRoles', value, false)}
													disabled={restartRequired}
													multi
													withTags
												>
											...
											<GridItem col={6} xs={12}>
												<ToggleInput
													...
													onChange={({ target: { checked } }: ToBeFixed) => setFieldValue('badWords', checked, false)}
													onLabel={getMessage('compontents.toogle.enabled')}
													offLabel={getMessage('compontents.toogle.disabled')}
													disabled={restartRequired}
												/>
											...
											{ isGQLPluginEnabled && (<GridItem col={6} xs={12}>
												<ToggleInput
													...
													onChange={({ target: { checked } }: ToBeFixed) => setFieldValue('gqlAuthEnabled', checked, false)}
													onLabel={getMessage('compontents.toogle.enabled')}
													offLabel={getMessage('compontents.toogle.disabled')}
													disabled={restartRequired}
												/>
