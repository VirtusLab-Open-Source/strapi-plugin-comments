...
const DiscussionThreadItemReviewAction = ({ 
    ...
    if (reviewFlowEnabled) {
        return (<>
            <IconButton onClick={handleReportsReviewClick} label={ getMessage('page.discover.table.reports.review') } icon={<ReviewIcon />} />
            <ReportsReviewModal 
                isVisible={reportsReviewVisible}
                isActionAsync={isLoading}
                allowedActions={{ canModerate, canAccessReports, canReviewReports }}
                onClose={handleReportsReviewClose}
                startActions={<Button onClick={handleReportsReviewClose} variant="tertiary">
                    { getMessage('compontents.confirmation.dialog.button.cancel', 'Cancel') }
                </Button>}
                endActions={canModerate && (<>
                    <Button onClick={handleBlockItemClick} variant="danger-light" startIcon={<LockIcon />} disabled={areBlockButtonsDisabled}>
                        { getMessage(`page.details.actions.comment.block`, 'Block comment') }
                    </Button>
                    { item.gotThread && (<Button onClick={handleBlockItemThreadClick} variant="danger" startIcon={<LockIcon />} disabled={areBlockButtonsDisabled}>
