...
export = () => ({
    ...
    config: {
      ...
      validator: (config: CommentsPluginConfig) => { 
        ...
        const enabledCollectionsValid: boolean = enabledCollections
          .filter((_: string) => !REGEX.uid.test(_)).length === 0;
        ...
        const moderatorRoles: PropType<CommentsPluginConfig, 'moderatorRoles'> = config[CONFIG_PARAMS.MODERATOR_ROLES] as PropType<CommentsPluginConfig, 'moderatorRoles'>;
        const moderatorRolesValid: boolean = moderatorRoles
          .filter((_: string) => !isString(_)).length === 0;
        ...
        const approvalFlow: PropType<CommentsPluginConfig, 'approvalFlow'> = config[CONFIG_PARAMS.APPROVAL_FLOW] as PropType<CommentsPluginConfig, 'approvalFlow'>;
        const approvalFlowValid: boolean = approvalFlow
          .filter((_: string) => !REGEX.uid.test(_)).length === 0;
        ...
        const entryLabels: PropType<CommentsPluginConfig, 'entryLabel'> = config[CONFIG_PARAMS.ENTRY_LABEL] as PropType<CommentsPluginConfig, 'entryLabel'>;
        const entryLabelKeysValid: boolean = Object.keys(entryLabels)
          .filter((_: string) => _ !== '*')
          .filter((_: string) => !(REGEX.uid.test(_) && isArray(entryLabels[_]) && !isEmpty(entryLabels[_]))).length === 0;
